...
sort_apps(Apps) ->
    AppDeps = [{rlx_app_info:name(App),
                rlx_app_info:applications(App) ++ rlx_app_info:included_applications(App)}
               || App <- Apps],
...
topo_apps_cycle_test() ->
    App1 = rlx_app_info:new(app1, "0.1", "/no-dir", [app2], [stdlib]),
    App2 = rlx_app_info:new(app2, "0.1", "/no-dir", [app1], []),
    Apps = [App1, App2],
    ?assertMatch({error, {_, {cycle, app1, [app2, app1]}}},
                 sort_apps(Apps)).

topo_apps_good_test() ->
    Apps = [rlx_app_info:new(app1, "0.1", "/no-dir", [app2, zapp1], [stdlib, kernel]),
...
topo_apps_1_test() ->
    Apps = [rlx_app_info:new(app0, "0.1", "/no-dir", [], [stdlib, dep1, dep2, dep3]),
...
topo_apps_2_test() ->
    Apps = [rlx_app_info:new(app1, "0.1", "/no-dir", [app2, app3, app4, app5,
                                                      stdlib, kernel],
                             []),
